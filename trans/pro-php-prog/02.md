...
![images](images/square.jpg) **注意:当出现错误时，PEAR 库中的**类将抛出`PEAR_Exception`类的对象。`PEAR_Exception`拥有普通`Exception`类的所有元素，变量`$trace`被添加到 lot 中。`PEAR_Exception`也会尝试显示堆栈跟踪，当它被抛出和捕获时。
...
![images](images/square.jpg) **注意**使用`foreach`循环的驱动数组时要小心。如果代码更改了驱动数组，可能会导致不可预测和意外的结果。
...
        $this->x = $x;
    }` `    function &get_x() {  // Observe the "&" in the function![images](images/U002.jpg)
declaration
        return $this->x;
    }
    function set_x($x) {
...
$a = new test6();
$b = &$a->get_x(); // $b is a reference to $x->a. It![images](images/U002.jpg)
circumvents protection
                                // provided by the "private"![images](images/U002.jpg)
qualifier.
print "b=$b\n";
$a->set_x(15);
print "b=$b\n";        // $b will change its value, after![images](images/U002.jpg)
calling "set_x"
$b++;
print '$a->get_x()='.$a->get_x() . "\n"; // $a->x will change![images](images/U002.jpg)
