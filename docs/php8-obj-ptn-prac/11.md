...
`interpret()`方法首先在其两个操作数属性上调用`interpret()`(如果您已经阅读了前一章，您可能会注意到我在这里创建了组合模式的一个实例)。一旦操作数已经运行，`interpret()`仍然需要获取它产生的值。它通过为每个属性调用`InterpreterContext::lookup()`来做到这一点。然后它调用`dointerpret()`，让子类来决定如何处理这些操作的结果。
