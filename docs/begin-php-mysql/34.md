...
MySQL 的两个存储引擎支持事务:InnoDB 和 NDB。InnoDB 在第 [25 章](25.html)中介绍过，NDP 不在本书讨论范围之内。本节解释应用于 InnoDB 的事务。它首先讨论了 InnoDB 处理器可用的系统要求和配置参数，最后给出了一个详细的使用示例和一系列使用 InnoDB 事务时需要记住的技巧。这一节为本章的最后一部分打下了基础，在这一部分中，您将学习如何将事务处理能力整合到您的 PHP 应用中。
