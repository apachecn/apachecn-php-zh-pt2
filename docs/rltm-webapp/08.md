...
`APP_URI`是app 的实际 URI。例如，如果应用位于`www.example.org`的网络根目录下，`APP_URI`将包含`http://www.example.org/;`，如果应用位于名为 realtime 的子目录中，`APP_URI`将存储`http://www.example.org/realtime/`。
