...
在我结束这一章之前，考虑一下:如果你想向后(向上)遍历这棵树呢？所以如果你启动一个子菜单，你想知道它的父菜单，父菜单的父菜单，等等。，现在这是一条单行道，从顶级巨型菜单开始。您可以用跟踪子节点的类似方式来保存对每个子节点的父节点的引用。这完全取决于您的应用的业务需求。如果决定存储父引用，一定要决定是希望对象只有一个父引用，还是允许多个父引用。当你冒险进入这个领域时，你可能会遇到除了复合模式之外的其他模式，这完全没问题，勇敢的灵魂，继续冒险吧！
