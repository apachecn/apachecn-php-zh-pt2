...
有时你想给一个对象增加额外的功能。这类似于装饰工。它们的不同之处在于，智能代理通常将其附加功能隐藏在公共接口下。装饰者可以添加额外的新公共方法，智能代理可能不会这样做。让我们向 miner 应用添加一个事件 fire/broadcast。这里唯一改变的是，当一个矿工开采一些东西时，你希望一个事件被触发。您的名为`LoudMouthMiner`的智能代理将为您处理此事。
